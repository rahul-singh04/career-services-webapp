...
const CandidateCard = ({ candidateId, fullName, bio, location, phoneNumber, linkedinProfile, githubProfile, jobsStateChange, jobId, status  , settrigger}) => {
    ...
    <div className="flex flex-row bg-white rounded-lg shadow-md p-6 mb-4 gap-4">
      ...
      <div className="flex flex-col w-full gap-2">
        <div className='flex mb-2 justify-between'>
          ...
          {role === 'ADMIN' && <Button size="sm" onClick={handleDelete} variant="gradient" color="red" className='h-8 my-2'>Delete</Button>}
          {role === 'EMPLOYER' && URLlocation.pathname === '/jobsAdded' && status === 'New' && <Button size="sm" onClick={handleInvite} variant="gradient" color="light-green" className='h-8 my-2'>Invite</Button>}
